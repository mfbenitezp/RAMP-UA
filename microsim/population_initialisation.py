...
class PopulationInitialisation:
    ...
    @classmethod
    def read_school_flows_data(cls, study_msoas: List[str]) -> Tuple(pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame):
        ...
        :return: A tuple of four dataframes. A dataframe of primary schools, a dataframe of secondary school, a dataframe of flows to primary schools
            and a dataframe of flows to secondary schools. No primary flows will flow to secondary schools and vice versa).
        :rtype: tuple
        ...
        if cls.quant_object is False:
            ...
            PopulationInitialisation._add_location_columns(schools, location_names=schools_names, location_ids=schools_ids)
    ...
    @classmethod
    def read_retail_flows_data(cls, study_msoas: List[str]) -> Tuple(pd.DataFrame, pd.DataFrame):
        """
                ...
                for lineno, raw_line in enumerate(f):
                    ...
                    if count == 1:  # OA and number of destinations
                        ...
                        for i in range(num_dests):  # Now add the flows
                            ...
                            row[dest - 1] = flow  # (-1 because destinations are numbered from 1, not 0)
                        assert len(
                            [x for x in row if x > 0]) == num_dests  # There should only be positive flows (no 0s)
                        row = [oa, oa_name] + row  # Insert the OA number and code (don't know this yet now)
